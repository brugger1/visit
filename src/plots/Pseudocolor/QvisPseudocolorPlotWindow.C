...
//
//   Kathleen Biagas, Tue Aug 27 09:12:53 PDT 2019
//   Move certain hide/show settings outside of the switch, handled
//   in separate methods that change the visibility of these controls based
//   on the rendering type(s) in effect.
//
// ****************************************************************************
...
void
QvisPseudocolorPlotWindow::pointTypeSettings()
{
    static int ptType = -1;
    bool canGlyphPoints = pcAtts->GetRenderSurfaces();

    if (canGlyphPoints && !pointControl->GetPointTypeEnabled())
    {
        pointControl->SetPointTypeEnabled(true);
        pointControl->SetPointType(ptType > 0 ? ptType: Point);
    }
    else if (!canGlyphPoints && pointControl->GetPointTypeEnabled())
    {
        ptType = pointControl->GetPointType();
        pointControl->SetPointType(Point);
        pointControl->SetPointTypeEnabled(false);
    }
}
